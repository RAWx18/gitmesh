...
import { agents as agentsTable, projects, heartbeatRuns } from "@gitmesh/data";
import { and, desc, eq, inArray, not, sql } from "@gitmesh/data";
import {
...
import { runClaudeLogin } from "@gitmesh/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@gitmesh/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@gitmesh/adapter-cursor-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@gitmesh/adapter-opencode-local/server";
...
export function agentRoutes(db: Db) {
  ...
  async function assertCanCreateAgentsForProject(req: Request, projectId: string) {
    ...
    if (req.actor.type === "operator") {
      ...
      const allowed = await access.canUser(projectId, req.actor.userId, "agents:create");
      if (!allowed) {
    ...
    if (!actorAgent || actorAgent.projectId !== projectId) {
      throw forbidden("Agent key cannot access another project");
    }
    const allowedByGrant = await access.hasPermission(projectId, "agent", actorAgent.id, "agents:create");
  ...
  async function assertCanReadConfigurations(req: Request, projectId: string) {
    return assertCanCreateAgentsForProject(req, projectId);
  }

  async function actorCanReadConfigurationsForProject(req: Request, projectId: string) {
    assertProjectAccess(req, projectId);
    if (req.actor.type === "operator") {
      if (req.actor.source === "local_implicit" || req.actor.isInstanceAdmin) return true;
      return access.canUser(projectId, req.actor.userId, "agents:create");
    }
    if (!req.actor.agentId) return false;
    const actorAgent = await svc.getById(req.actor.agentId);
    if (!actorAgent || actorAgent.projectId !== projectId) return false;
    const allowedByGrant = await access.hasPermission(projectId, "agent", actorAgent.id, "agents:create");
    return allowedByGrant || canCreateAgents(actorAgent);
  }

  async function assertCanUpdateAgent(req: Request, targetAgent: { id: string; projectId: string }) {
    assertProjectAccess(req, targetAgent.projectId);
    if (req.actor.type === "operator") return;
    if (!req.actor.agentId) throw forbidden("Agent authentication required");

    const actorAgent = await svc.getById(req.actor.agentId);
    if (!actorAgent || actorAgent.projectId !== targetAgent.projectId) {
      throw forbidden("Agent key cannot access another project");
    }
  ...
  async function resolveProjectIdForAgentReference(req: Request): Promise<string | null> {
    const projectIdQuery = req.query.projectId;
    const requestedProjectId =
      typeof projectIdQuery === "string" && projectIdQuery.trim().length > 0
        ? projectIdQuery.trim()
        : null;
    if (requestedProjectId) {
      assertProjectAccess(req, requestedProjectId);
      return requestedProjectId;
    }
    if (req.actor.type === "agent" && req.actor.projectId) {
      return req.actor.projectId;
    }
    return null;
  }
  ...
  async function normalizeAgentReference(req: Request, rawId: string): Promise<string> {
    ...
    const projectId = await resolveProjectIdForAgentReference(req);
    if (!projectId) {
      throw unprocessable("Agent shortname lookup requires projectId query parameter");
    }

    const resolved = await svc.resolveByReference(projectId, raw);
    if (resolved.ambiguous) {
      throw conflict("Agent shortname is ambiguous in this project. Use the agent ID.");
    }
    if (!resolved.agent) {
  ...
  function ensureGatewayDeviceKey(
    ...
    if (adapterType !== "gateway") return adapterConfig;
  ...
  async function assertAdapterConfigConstraints(
    ...
    const { config: runtimeConfig } = await secretsSvc.resolveAdapterConfigForRuntime(projectId, adapterConfig);
  ...
  async function assertCanManageInstructionsPath(req: Request, targetAgent: { id: string; projectId: string }) {
    assertProjectAccess(req, targetAgent.projectId);
    if (req.actor.type === "operator") return;
    if (!req.actor.agentId) throw forbidden("Agent authentication required");
  ...
  router.get("/projects/:projectId/adapters/:type/models", async (req, res) => {
  ...
  router.post(
    ...
    async (req, res) => {
      const projectId = req.params.projectId as string;
      const type = req.params.type as string;
      await assertCanReadConfigurations(req, projectId);
  ...
  router.get("/projects/:projectId/agents", async (req, res) => {
    ...
    const canReadConfigs = await actorCanReadConfigurationsForProject(req, projectId);
  ...
  router.get("/projects/:projectId/org", async (req, res) => {
    const projectId = req.params.projectId as string;
    assertProjectAccess(req, projectId);
    const tree = await svc.orgForProject(projectId);
    const leanTree = tree.map((node) => toLeanOrgNode(node as Record<string, unknown>));
    res.json(leanTree);
  });

  router.get("/projects/:projectId/agent-configurations", async (req, res) => {
    const projectId = req.params.projectId as string;
    await assertCanReadConfigurations(req, projectId);
    const rows = await svc.list(projectId);
  ...
  router.get("/agents/:id", async (req, res) => {
    ...
    if (req.actor.type === "agent" && req.actor.agentId !== id) {
      const canRead = await actorCanReadConfigurationsForProject(req, agent.projectId);
      if (!canRead) {
        const chainOfCommand = await svc.getChainOfCommand(agent.id);
  ...
  router.get("/agents/:id/configuration", async (req, res) => {
    ...
    await assertCanReadConfigurations(req, agent.projectId);
  ...
  router.post("/projects/:projectId/agent-enables", validate(createAgentEnableSchema), async (req, res) => {
    const projectId = req.params.projectId as string;
    await assertCanCreateAgentsForProject(req, projectId);
    ...
    const requiresApproval = project.requireOperatorApprovalForNewAgents;
    const status = requiresApproval ? "pending_approval" : "idle";
    const agent = await svc.create(projectId, {
      ...normalizedHireInput,
      status,
      spentMonthlyCents: 0,
    ...
    if (requiresApproval) {
      ...
      approval = await approvalsSvc.create(projectId, {
        type: "enable_agent",
  ...
  router.post("/projects/:projectId/agents", validate(createAgentSchema), async (req, res) => {
    const projectId = req.params.projectId as string;
    assertProjectAccess(req, projectId);
  ...
  router.post(
    "/projects/:projectId/agents/bulk-delete",
    validate(bulkDeleteAgentsSchema),
    async (req, res) => {
      assertBoard(req);
      const projectId = req.params.projectId as string;
      assertProjectAccess(req, projectId);

      // Soft-delete (terminate) rather than hard-delete: the UI already
      // labels this action "terminate" and filters terminated workers
      // out of the roster. Hard-delete would violate the activity_log
      // → agents FK for any worker that has run history.
      ...
      for (const id of ids) {
        ...
        const terminated = await svc.terminate(id);
        if (terminated) {
          deletedIds.push(terminated.id);
          const actor = getActorInfo(req);
          await logActivity(db, {
            projectId,
            actorType: actor.actorType,
            ...
            details: { name: terminated.name, bulk: true },
          });
        }
      }

      res.json({ ok: true as const, deletedIds, skippedIds });
    },
  );
  ...
  router.patch("/agents/:id/permissions", validate(updateAgentPermissionsSchema), async (req, res) => {
    ...
    if (req.actor.type === "agent") {
      ...
      if (!actorAgent || actorAgent.projectId !== existing.projectId) {
  ...
  router.post("/agents/:id/claude-login", async (req, res) => {
    ...
    const { config: runtimeConfig } = await secretsSvc.resolveAdapterConfigForRuntime(agent.projectId, config);
  ...
  router.get("/projects/:projectId/heartbeat-runs", async (req, res) => {
    const projectId = req.params.projectId as string;
    assertProjectAccess(req, projectId);
    const agentId = req.query.agentId as string | undefined;
    const limitParam = req.query.limit as string | undefined;
    const limit = limitParam ? Math.max(1, Math.min(1000, parseInt(limitParam, 10) || 200)) : undefined;
    const runs = await heartbeat.list(projectId, agentId, limit);
    res.json(runs);
  });

  router.get("/projects/:projectId/live-runs", async (req, res) => {
    const projectId = req.params.projectId as string;
    assertProjectAccess(req, projectId);
  ...
  router.get("/heartbeat-runs/:runId/events", async (req, res) => {
    ...
    assertProjectAccess(req, run.projectId);
  ...
  router.get("/issues/:issueId/live-runs", async (req, res) => {
    ...
    const liveRuns = await db
      ...
      .where(
        and(
          eq(heartbeatRuns.projectId, issue.projectId),
          inArray(heartbeatRuns.status, ["queued", "running"]),
