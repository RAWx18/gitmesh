...
if [ "$promote" = true ]; then
  ...
  if [ "$dry_run" = true ]; then
    ...
    echo ""
    echo "Verify: npm view gitmesh-agents@latest version"
    echo ""
    echo "To push:"
...
echo "  ✓ All packages built (including UI + playbooks)"
...
if [ "$canary" = true ]; then
  if [ "$dry_run" = true ]; then
    ...
    echo ""
    echo "Verify: npm view gitmesh-agents@canary version"
    echo ""
  ...
  echo "  git push && git push origin v$NEW_VERSION"
  echo ""
  echo "GitHub Release: https://github.com/cryppadotta/gitmesh-agents/releases/tag/v$NEW_VERSION"
fi
