# GitMesh Agents Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for GitMesh Agents's plugin and extension architecture.
It expands the brief plugin notes in [doc/architecture.md](../architecture.md) and should be read alongside the comparative analysis in [doc/plugins/ideas-from-opencode.md](./ideas-from-opencode.md).

This is not part of the V1 implementation contract in [doc/v1-spec.md](../v1-spec.md).
...
GitMesh Agents plugin design is based on the following assumptions:

1. GitMesh Agents is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Projects" remain core GitMesh Agents business objects, but they are not plugin trust boundaries.
4. Operator governance, approval gates, budget hard-stops, and core task invariants remain owned by GitMesh Agents core.
...
2. Let plugins add major capabilities without editing GitMesh Agents core.
...
4. Depend on project-local plugin folders such as `.gitmesh-agents/plugins`.
...
The single GitMesh Agents deployment an operator installs and controls.

### 5.2 Project

A first-class GitMesh Agents business object inside the instance.
...
A trusted in-process extension loaded directly by GitMesh Agents core.
...
An installable instance-wide extension package loaded through the GitMesh Agents plugin runtime.
...
GitMesh Agents has two extension classes.
...
GitMesh Agents already has a concrete workspace model:
...
There is no per-project install table and no per-project enable/disable switch.
...
- mappings from project A to Linear team X and project B to Linear team Y
- one global git plugin install
- per-project workspace state stored under `project_workspace`

## 8.1 On-Disk Layout

Plugins live under the GitMesh Agents instance directory.

Suggested layout:

- `~/.gitmesh-agents/instances/default/plugins/package.json`
- `~/.gitmesh-agents/instances/default/plugins/node_modules/`
- `~/.gitmesh-agents/instances/default/plugins/.cache/`
- `~/.gitmesh-agents/instances/default/data/plugins/<plugin-id>/`

The package install directory and the plugin data directory are separate.

## 8.2 Operator Commands

GitMesh Agents should add CLI commands:

- `pnpm gitmesh-agents plugin list`
- `pnpm gitmesh-agents plugin install <package[@version]>`
- `pnpm gitmesh-agents plugin uninstall <plugin-id>`
- `pnpm gitmesh-agents plugin upgrade <plugin-id> [version]`
- `pnpm gitmesh-agents plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@gitmesh-agents/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```json
{
  "name": "@gitmesh-agents/plugin-linear",
  "version": "0.1.0",
  "gitmesh-agentsPlugin": {
...
```ts
export interface GitMesh AgentsPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
  ...
  minimumGitMesh AgentsVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that GitMesh Agents agents can use during runs.
...
- GitMesh Agents server starts one worker process per installed plugin
...
Receives one typed GitMesh Agents domain event.
...
- context (project id, project id, entity id, etc.)
...
- run context: agent ID, run ID, project ID, project ID
...
export function definePlugin(definition: PluginDefinition): GitMesh AgentsPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: GitMesh AgentsPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@gitmesh/plugin-sdk/ui";

export function DashboardWidget({ context }: PluginWidgetProps) {
  const { data, loading } = usePluginData("sync-health", { projectId: context.projectId });
...
2. The host mounts the plugin's `DashboardWidget` component into the dashboard widget slot, passing `context` (current project, user, etc.) and the bridge.
...
### 19.0.1 Plugin UI SDK (`@gitmesh/plugin-sdk/ui`)
...
## 19.2 Project-Context Routes

- `/:projectPrefix/plugins/:pluginId`

These routes exist because the operator UI is organized around projects even though plugin installation is global.
...
- `/:projectPrefix/<entity>/:id?tab=<plugin-tab-id>`
...
## 19.6 Shared Components In `@gitmesh/plugin-sdk/ui`
...
1. Core GitMesh Agents data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing GitMesh Agents objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual GitMesh Agents product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if GitMesh Agents later decides git state is core product data, it should become a first-party table too
...
- `scope_kind` enum: `instance | project | project | project_workspace | agent | issue | goal | run`
...
2. Secret refs resolve through the existing GitMesh Agents secret provider system.
...
## 24.3 Project-Context Plugin Page

Each plugin may expose a project-context main page:
...
6. The operator may force-purge immediately via CLI: `pnpm gitmesh-agents plugin purge <plugin-id>`.
...
### 27.1 `@gitmesh/plugin-test-harness`
...
```ts
import { createTestHarness } from "@gitmesh/plugin-test-harness";
...
const data = await harness.getData("sync-health", { projectId: "comp-1" });
expect(data.syncedCount).toBeGreaterThan(0);
```

### 27.2 Local Plugin Development

For developing a plugin against a running GitMesh Agents instance:

- The operator installs the plugin from a local path: `pnpm gitmesh-agents plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-gitmesh-agents-plugin`) that scaffolds:

- `package.json` with correct `gitmesh-agentsPlugin` keys
...
- `@gitmesh-agents/plugin-workspace-files`
- `@gitmesh-agents/plugin-terminal`
- `@gitmesh-agents/plugin-git`
- `@gitmesh-agents/plugin-linear`
- `@gitmesh-agents/plugin-github-issues`
- `@gitmesh-agents/plugin-grafana`
- `@gitmesh-agents/plugin-runtime-processes`
- `@gitmesh-agents/plugin-stripe`
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@gitmesh/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@gitmesh/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@gitmesh/plugin-sdk/ui`
...
- `@gitmesh/plugin-test-harness`
- `create-gitmesh-agents-plugin` starter template
...
GitMesh Agents should not implement a generic in-process hook bag modeled directly after local coding tools.

GitMesh Agents should implement:
...
That is the complete target design for the GitMesh Agents plugin system.
