...
GitMesh Agents V1 explicitly excludes a plugin framework in [doc/v1-spec.md](../v1-spec.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a GitMesh Agents-shaped design.

Assumption for this document: GitMesh Agents is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Projects" are still first-class GitMesh Agents objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into GitMesh Agents.

The main conclusion is:

- GitMesh Agents should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- GitMesh Agents should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- GitMesh Agents should use multiple extension classes instead of one generic plugin bag:
...
If GitMesh Agents does this well, the examples you listed become straightforward:
...
Relevant GitMesh Agents files reviewed for current extension seams:
...
GitMesh Agents should absolutely do this.
...
GitMesh Agents should copy this discipline.
...
GitMesh Agents should adopt this style for plugin actions, automations, and UI schemas.
...
GitMesh Agents can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for GitMesh Agents too.

## What GitMesh Agents Should Not Copy Directly
...
GitMesh Agents is a control plane for an operator-managed instance with project objects.
...
GitMesh Agents is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

GitMesh Agents should not auto-load arbitrary code from a workspace repo like `.gitmesh-agents/plugins` or project directories.
...
For GitMesh Agents, equivalent hooks into:
...
GitMesh Agents should not allow plugins to silently replace:
...
For GitMesh Agents it would be risky because it combines:
...
GitMesh Agents should require an explicit operator install step.

## Why GitMesh Agents Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | GitMesh Agents |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with project objects |
| Trust assumption | local power user on own machine | operator managing one trusted GitMesh Agents instance |
| Failure blast radius | local session/runtime | entire project control plane |
...
That means GitMesh Agents should borrow the good ideas from `opencode` but use a stricter architecture.

## GitMesh Agents Already Has Useful Pre-Plugin Seams

GitMesh Agents has several extension-like seams already:
...
## Recommended GitMesh Agents Plugin Model
...
| UI contribution | dashboard widgets, settings forms, project panels | plugin-shipped React bundles in host extension slots via bridge | medium | plugins own their rendering; host controls slot placement and bridge access |
...
GitMesh Agents already has this pattern implicitly:
...
- `plugin` means integration code that talks to GitMesh Agents through a typed plugin protocol and capability model
...
- read instance state, including project-bound business records when relevant
- register webhooks
- run scheduled jobs
- contribute tools that agents can use during runs
- write plugin-owned state
- add additive UI surfaces
- invoke explicit GitMesh Agents actions through the API
...
GitMesh Agents should treat plugin installation as a global instance-level action.

Examples:

- install `@gitmesh-agents/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over GitMesh Agents objects if one project maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

GitMesh Agents already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for GitMesh Agents too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@gitmesh-agents/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@gitmesh/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running GitMesh Agents instance.

A `create-gitmesh-agents-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how GitMesh Agents achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the GitMesh Agents server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. GitMesh Agents's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.
...
## A Concrete SDK Shape For GitMesh Agents

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@gitmesh/plugin-sdk";

export default definePlugin({
  id: "@gitmesh-agents/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit GitMesh Agents entities
    });
    ...
    ctx.events.on("plugin.@gitmesh-agents/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@gitmesh/plugin-sdk/ui";

export function DashboardWidget({ context }: PluginWidgetProps) {
  const { data, loading, error } = usePluginData("sync-health", { projectId: context.projectId });
  ...
  return (
    <ErrorBoundary fallback={<div>Widget failed to render</div>}>
      ...
      <button onClick={() => resync({ projectId: context.projectId })}>Resync Now</button>
    </ErrorBoundary>
  );
}
```
...
- explicit global config with optional project mappings
...
Any GitMesh Agents plugin system has to preserve core control-plane invariants from the repo docs.

That means:

- plugin install is global to the instance
- "projects" remain business objects in the API and data model, not tenant boundaries
...
If it needs mappings over specific GitMesh Agents objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@gitmesh-agents/plugin-linear`)
...
One broken plugin must not break the rest of the project.
...
- reuse core tables when the data is clearly part of GitMesh Agents itself
...
If a concept is becoming part of GitMesh Agents's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of GitMesh Agents itself
...
### 3. Scope plugin data to GitMesh Agents objects before adding custom schemas

A lot of plugin data naturally hangs off existing GitMesh Agents objects:

- project workspace plugin state should often scope to `project` or `project_workspace`
- issue sync state should scope to `issue`
- metrics widgets may scope to `project`, `project`, or `goal`
...
If GitMesh Agents eventually needs extension-owned tables, I would only allow that for:
...
- `id`
- `plugin_id`
- `scope_kind` (`instance | project | project | project_workspace | agent | issue | goal | run`)
- `scope_id` nullable
- `namespace`
- `state_key`
- `value_json`
...
| Stripe revenue tracking | connector plugin | secret refs, scheduled sync, project metrics API | strong plugin candidate |

# Plugin Examples

## Workspace File Browser

Package idea: `@gitmesh-agents/plugin-workspace-files`
...
- understanding repo layout for a project
- inspecting agent home workspaces in local-trusted mode

### UX

- Settings page: `/settings/plugins/workspace-files`
- Main page: `/:projectPrefix/plugins/workspace-files`
- Project tab: `/:projectPrefix/projects/:projectId?tab=files`
- Optional issue tab: `/:projectPrefix/issues/:issueId?tab=files`
- Optional agent tab: `/:projectPrefix/agents/:agentId?tab=workspace`
...
Package idea: `@gitmesh-agents/plugin-terminal`
...
- Main page: `/:projectPrefix/plugins/terminal`
- Project tab: `/:projectPrefix/projects/:projectId?tab=terminal`
- Optional agent tab: `/:projectPrefix/agents/:agentId?tab=terminal`
- Optional run tab: `/:projectPrefix/agents/:agentId/runs/:runId?tab=terminal`
...
Package idea: `@gitmesh-agents/plugin-git`
...
- Main page: `/:projectPrefix/plugins/git`
- Project tab: `/:projectPrefix/projects/:projectId?tab=git`
- Optional issue tab: `/:projectPrefix/issues/:issueId?tab=git`
- Optional agent tab: `/:projectPrefix/agents/:agentId?tab=git`
...
- Operator opens a project page and reviews the diff for that project's workspace without leaving GitMesh Agents.
- Operator reviews the diff after a run without leaving GitMesh Agents.
...
- optional `events.emit` (e.g. `plugin.@gitmesh-agents/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@gitmesh-agents/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@gitmesh-agents/plugin-linear`

This plugin syncs GitMesh Agents work with Linear. It is useful for:

- importing backlog from Linear
- linking GitMesh Agents issues to Linear issues
- syncing status, comments, and assignees
- mapping project goals/projects to external product planning
- giving operator operators a single place to see sync health

### UX

- Settings page: `/settings/plugins/linear`
- Main page: `/:projectPrefix/plugins/linear`
- Dashboard widget: `/:projectPrefix/dashboard`
- Optional issue tab: `/:projectPrefix/issues/:issueId?tab=linear`
- Optional project tab: `/:projectPrefix/projects/:projectId?tab=linear`
...
- Plugin settings:
  ...
  - status mapping between GitMesh Agents and Linear
...
- Operator enables the plugin, maps a Linear team, and imports a backlog into GitMesh Agents.
- GitMesh Agents issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@gitmesh-agents/plugin-github-issues`

This plugin syncs GitMesh Agents issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to GitMesh Agents issues
- tracking cross-repo work from inside one project view
- bridging engineering workflow with GitMesh Agents task governance

### UX

- Settings page: `/settings/plugins/github-issues`
- Main page: `/:projectPrefix/plugins/github-issues`
- Dashboard widget: `/:projectPrefix/dashboard`
- Optional issue tab: `/:projectPrefix/issues/:issueId?tab=github`
- Optional project tab: `/:projectPrefix/projects/:projectId?tab=github`
...
- Plugin settings:
  ...
  - whether new GitMesh Agents issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active GitMesh Agents issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Operator imports GitHub Issues for a repo into GitMesh Agents.
- GitHub webhooks update status/comment state in GitMesh Agents.
- A PR is linked back to the GitMesh Agents issue so the operator can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@gitmesh-agents/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@gitmesh-agents/plugin-grafana`

This plugin surfaces external metrics and dashboards inside GitMesh Agents. It is useful for:

- project KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating GitMesh Agents issues from anomalous metrics

### UX

- Settings page: `/settings/plugins/grafana`
- Main page: `/:projectPrefix/plugins/grafana`
- Dashboard widgets: `/:projectPrefix/dashboard`
- Optional goal tab: `/:projectPrefix/goals/:goalId?tab=metrics`
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full GitMesh Agents plugin page
- Full metrics page:
...
- Operator sees service degradation or business KPI movement directly on the GitMesh Agents dashboard.
- Operator clicks into the full metrics page to inspect the relevant Grafana panels.
- Operator creates a GitMesh Agents issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of GitMesh Agents core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@gitmesh-agents/plugin-runtime-processes`
...
- Main page: `/:projectPrefix/plugins/runtime-processes`
- Dashboard widget: `/:projectPrefix/dashboard`
- Process detail page: `/:projectPrefix/plugins/runtime-processes/:processId`
- Project tab: `/:projectPrefix/projects/:projectId?tab=processes`
- Optional agent tab: `/:projectPrefix/agents/:agentId?tab=processes`
...
Package idea: `@gitmesh-agents/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into GitMesh Agents. It is useful for:

- showing MRR and churn next to project goals
...
- Main page: `/:projectPrefix/plugins/stripe`
- Dashboard widgets: `/:projectPrefix/dashboard`
- Optional project/goal metric tabs if those surfaces exist later
...
- Revenue widgets appear on the main dashboard and can be linked to project goals.
- Failed payment spikes or churn events can generate GitMesh Agents issues for follow-up.
...
- Stripe data should stay additive to GitMesh Agents core
...
- global plugin config and optional project-mapping storage
- secret ref access
- typed domain event subscription
- scheduled jobs
- webhook endpoints
- activity logging helpers
- plugin UI bundle loading, host bridge, `@gitmesh/plugin-sdk/ui`
...
- `@gitmesh/plugin-test-harness` and `create-gitmesh-agents-plugin` starter template
...
GitMesh Agents should not implement "an OpenCode-style generic in-process hook system."
GitMesh Agents should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In GitMesh Agents
