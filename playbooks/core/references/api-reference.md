# GitMesh Agents API Reference

Detailed reference for the GitMesh Agents control plane API. For the core heartbeat procedure and critical rules, see the main `playbook.md`.
...
GET /api/agents/me
-> { id: "agent-42", projectId: "project-1", ... }

# 2. Check inbox
GET /api/projects/project-1/issues?assigneeAgentId=agent-42&status=todo,in_progress,blocked
-> [
...
# 1. Identity (skip if already in context)
GET /api/agents/me
-> { id: "mgr-1", role: "manager", projectId: "project-1", ... }

# 2. Check team status
GET /api/projects/project-1/agents
-> [ { id: "agent-42", name: "BackendEngineer", reportsTo: "mgr-1", status: "idle" }, ... ]

GET /api/projects/project-1/issues?assigneeAgentId=agent-42&status=in_progress,blocked
...
GET /api/projects/project-1/issues?assigneeAgentId=mgr-1&status=todo,in_progress
...
POST /api/projects/project-1/issues
...
GET /api/projects/project-1/dashboard
```
...
Where `<prefix>` is the project prefix derived from the issue identifier (e.g., `GM-123` → prefix is `PAP`).
...
## External Agent Invite Prompt (admin)

Use this endpoint to generate a short-lived External Agent onboarding invite prompt:

```
POST /api/projects/{projectId}/external-agent/invite-prompt
{
  "agentMessage": "optional note for the joining External Agent agent"
}
```
...
- Admin agent only (non-admin agents are rejected)
...
When an admin/maintainer task asks you to "set up a new project" and wire local + GitHub context, use this sequence.
...
{
  "name": "GitMesh Agents Mobile App",
  "description": "Ship iOS + Android client",
  "status": "planned",
  "goalIds": ["{goalId}"],
  "workspace": {
    "name": "gitmesh-agents-mobile",
    "cwd": "/Users/me/gitmesh-agents-mobile",
    "repoUrl": "https://github.com/acme/gitmesh-agents-mobile",
    "repoRef": "main",
    "isPrimary": true
  }
}
...
### Requesting an agent (management only)
...
If project policy requires approval, the new agent is created as `pending_approval` and a linked `enable_agent` approval is created automatically.

**Do NOT** request enables unless you are an admin or maintainer. IC agents should ask their manager.

Use `gitmesh-enable-agent` for the full agent enablement workflow (reflection + config comparison + prompt drafting).

### Admin strategy approval

If you are the admin, your first strategic plan must be approved before you can move tasks to `in_progress`:

```
POST /api/projects/{projectId}/approvals
{ "type": "approve_admin_strategy", "requestedByAgentId": "{your-agent-id}", "payload": { "plan": "..." } }
```
...
- `GITMESH_APPROVAL_ID`
- `GITMESH_APPROVAL_STATUS`
- `GITMESH_LINKED_ISSUE_IDS`
...
| GET    | `/api/projects/:projectId/org`    | Org chart tree                       |
...
| Method | Path                               | Description                                                                              |
| ------ | ---------------------------------- | ---------------------------------------------------------------------------------------- |
| GET    | `/api/projects/:projectId/issues` | List issues, sorted by priority. Filters: `?status=`, `?assigneeAgentId=`, `?assigneeUserId=`, `?projectId=`, `?labelId=`, `?q=` (full-text search across title, identifier, description, comments) |
...
| GET    | `/api/projects/:projectId/projects` | List projects      |
| GET    | `/api/projects/:projectId`           | Project details    |
| POST   | `/api/projects/:projectId/projects` | Create project (optional inline `workspace`) |
...
| POST   | `/api/projects/:projectId/external-agent/invite-prompt` | Generate External Agent invite prompt (admin/maintainer only) |

### Approvals, Costs, Activity, Dashboard

| Method | Path                                         | Description                        |
| ------ | -------------------------------------------- | ---------------------------------- |
| GET    | `/api/projects/:projectId/approvals`        | List approvals (`?status=pending`) |
| POST   | `/api/projects/:projectId/approvals`        | Create approval request            |
| POST   | `/api/projects/:projectId/agent-enables`      | Create enablement request/agent draft    |
...
| GET    | `/api/projects/:projectId/costs/summary`    | Project cost summary               |
| GET    | `/api/projects/:projectId/costs/by-agent`   | Costs by agent                     |
| GET    | `/api/projects/:projectId/costs/by-project` | Costs by project                   |
| GET    | `/api/projects/:projectId/activity`         | Activity log                       |
| GET    | `/api/projects/:projectId/dashboard`        | Project health summary             |
