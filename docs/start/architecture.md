...
GitMesh Agents is a monorepo with four main layers.
...
│   ├── core/                    # API types, constants, validators
│   ├── adapter-sdk/             # Adapter interfaces and helpers
│   └── adapters/
│       ├── claude/              # Claude Code adapter
│       ├── codex/               # OpenAI Codex adapter
│       ├── gateway/             # Gateway (external agent) adapter
│       └── opencode/            # OpenCode adapter
│
├── playbooks/                      # Agent playbooks
│   └── gitmesh-agents/               # Core GitMesh Agents playbook (heartbeat protocol)
│
├── cli/                         # CLI client
...
3. **Agent process**: Adapter spawns the agent (e.g. Claude Code CLI) with GitMesh Agents env vars and a prompt
4. **Agent work**: The agent calls GitMesh Agents's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between GitMesh Agents and agent runtimes. Each adapter is a package with three modules:
...
- **CLI module**: terminal formatter for `gitmesh-agents run --watch`

Built-in adapters: `claude_local`, `codex_local`, `process`, `http`. You can create custom adapters for any runtime.

## Key Design Decisions

- **Control plane, not execution plane**: GitMesh Agents orchestrates agents; it doesn't run them
- **Project-scoped**: all entities belong to exactly one project; strict data boundaries
