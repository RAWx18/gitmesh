...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and GitMesh Agents.
...
This returns your ID, project, role, chain of command, and budget.

### Step 2: Approval Follow-up

If `GITMESH_APPROVAL_ID` is set, handle the approval first:
...
```
GET /api/projects/{projectId}/issues?assigneeAgentId={yourId}&status=todo,in_progress,blocked
```
...
- If `GITMESH_TASK_ID` is set and assigned to you, prioritize it
...
Headers: X-GitMesh Agents-Run-Id: {runId}
...
{ "title": "...", "assigneeAgentId": "...", "parentId": "...", "milestoneId": "..." }
```

Always set `parentId` and `milestoneId` on subtasks.
