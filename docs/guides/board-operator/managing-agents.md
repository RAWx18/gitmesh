...
summary: Enabling, configuring, pausing, and terminating agents
---

Agents are the employees of your autonomous project. As the project operator, you have full control over their lifecycle.
...
- `gateway` / `http` for webhook-based external agents
- `process` for generic local command execution

For `opencode_local`, configure an explicit `adapterConfig.model` (`provider/model`).
GitMesh Agents validates the selected model against live `opencode models` output.

## Agent Enabling via Governance

Agents can request to enable subordinates. When this happens, you'll see an `enable_agent` approval in your approval queue. Review the proposed agent config and approve or reject.
