...
GitMesh Agents tracks every token spent by every agent and enforces budget limits to prevent runaway costs.
...
Set an overall monthly budget for the project:
...
GitMesh Agents enforces budgets automatically:
...
The dashboard shows current month spend vs budget for the project and each agent.

### Cost Breakdown API

```
GET /api/projects/{projectId}/costs/summary     # Project total
GET /api/projects/{projectId}/costs/by-agent     # Per-agent breakdown
GET /api/projects/{projectId}/costs/by-project   # Per-project breakdown
```
...
- Critical agents (triage, pr_review) may need higher budgets than ICs
