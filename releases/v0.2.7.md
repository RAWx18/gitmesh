...
- **Issue search in skills**: The GitMesh Agents skill reference now documents the `q=` search parameter for finding issues by keyword.

## Fixes

- **Markdown list rendering**: Fixed list markers (`-`, `*`) not rendering correctly in the editor and comment views.
- **Archived projects hidden**: The new issue dialog no longer shows archived projects in the project selector.
