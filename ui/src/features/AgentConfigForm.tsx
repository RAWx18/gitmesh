...
import { AGENT_ADAPTER_TYPES } from "@gitmesh/core";
import type {
  Agent,
...
import { MinimaxAdvancedFields } from "../adapters/minimax/config-fields";
...
// Canonical type lives in @gitmesh/adapter-sdk; re-exported here
// so existing imports from this file keep working.
export type { CreateConfigValues } from "@gitmesh/adapter-sdk";
...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const { data: availableSecrets = [] } = useQuery({
    queryKey: selectedProjectId ? queryKeys.secrets.list(selectedProjectId) : ["secrets", "none"],
    queryFn: () => secretsApi.list(selectedProjectId!),
    enabled: Boolean(selectedProjectId),
  });

  const createSecret = useMutation({
    mutationFn: (input: { name: string; value: string }) => {
      if (!selectedProjectId) throw new Error("Select a project to create secrets");
      return secretsApi.create(selectedProjectId, input);
    },
    onSuccess: () => {
      if (!selectedProjectId) return;
      queryClient.invalidateQueries({ queryKey: queryKeys.secrets.list(selectedProjectId) });
    },
  });

  const uploadMarkdownImage = useMutation({
    mutationFn: async ({ file, namespace }: { file: File; namespace: string }) => {
      if (!selectedProjectId) throw new Error("Select a project to upload images");
      return assetsApi.uploadImage(selectedProjectId, file, namespace);
    },
  });
  ...
  const {
    ...
    queryKey: selectedProjectId
      ? queryKeys.agents.adapterModels(selectedProjectId, adapterType)
      : ["agents", "none", "adapter-models", adapterType],
    queryFn: () => agentsApi.adapterModels(selectedProjectId!, adapterType),
    enabled: Boolean(selectedProjectId),
  });
  ...
  const testEnvironment = useMutation({
    mutationFn: async () => {
      if (!selectedProjectId) {
        throw new Error("Select a project to test adapter environment");
      }
      return agentsApi.testEnvironment(selectedProjectId, adapterType, {
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          ...
          <Button
            ...
            disabled={testEnvironment.isPending || !selectedProjectId}
          >
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              {(adapterType as string) === "minimax" && (
                <MinimaxAdvancedFields {...adapterFieldProps} />
              )}
...
const ENABLED_ADAPTER_TYPES = new Set(["claude_local", "codex_local", "opencode_local", "cursor", "minimax"]);
...
function EnvVarEditor({
  ...
  onCreateSecret: (name: string, value: string) => Promise<ProjectSecret>;
  ...
  return (
    <div className="space-y-1.5">
      ...
      <p className="text-[11px] text-muted-foreground/60">
        GITMESH_* variables are injected automatically at runtime.
      </p>
    </div>
  );
}
